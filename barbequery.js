...

{
        ...
        createInstance(initObj) {
            ...
            for (let i = 0; i < this.children.length; i++) {
                ...
                if (childNode instanceof VirtualEl) {
                    ...
                    let startIdx;
                    while ((startIdx = textContent.indexOf("\\{")) !== -1) {
                        let endIdx = textContent.indexOf("}", startIdx + 2);
                        const prop = textContent.slice(startIdx + 2, endIdx);
                        console.log(initObj, startIdx, endIdx, prop)
                        textContent = textContent.slice(0, startIdx) + initObj[prop] + textContent.slice(endIdx + 1);
                    }
    ...
    function instantiateComponent(component, initObj) {
        ...
        if (component.isNative) {
            for (let i = 0; i < template.children.length; i++) {
                ...
                if (childNode instanceof VirtualEl && !(childNode.tag === "script" && childNode.attributes["type"] === "methods")) {
                    const nativeComponent = childNode.createInstance(initObj);
            ...
            createdElement = template.createInstance(initObj);
        }
        ...
        for (let i = 0; i < selectors.length; i++) {
            ...
            switch (select.charAt(0)) {
                ...
                default:
                    ...
                    if (component) {
                        return instantiateComponent(component, initObj);
                    } else {
